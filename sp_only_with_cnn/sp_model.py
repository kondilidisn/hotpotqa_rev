...
class SPModel(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        #self.linear_sp = nn.Linear(config.sent_limit*config.hidden*2, config.hidden*2)
        #self.out = nn.Sigmoid()
        self.conv = convs.CNN_Text()
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, sentence_length, start_mapping, end_mapping):
        ...
        # context_output = self.rnn(context_output, sentence_length)
        context_output = self.rnn(context_output)
        ques_output = self.rnn(ques_output)

        output = self.qc_att(context_output, ques_output, ques_mask)
        output = self.linear_1(output)

        # output_t = self.rnn_2(output, sentence_length)
        output_t = self.rnn_2(output)
        output_t = self.self_att(output_t, output_t, context_mask)
        output_t = self.linear_2(output_t)

        output = output + output_t

        # sp_output = self.rnn_sp(output, sentence_length)
        sp_output = self.rnn_sp(output)


        #print('A ! Sp_output size :', sp_output.size())





        #print('start_output :', start_mapping.size(), 'sp_output.size()=', sp_output.size())
        ...
        # print('start_output :', start_output.shape)
        # print('end_output :', end_output.shape)
        # print('sp_output[:,:,self.hidden:] :', sp_output[:,:,self.hidden:].shape)

        #sp_output = torch.cat([start_output, end_output], dim=-1)


        #print('B * Sp_output size :', sp_output.size())

        # print('sp_output concatenated (start_mapping , end_mapping):', sp_output.shape)

        #sp_output_t = self.linear_sp(sp_output.contiguous().view(sp_output.size(0),-1))
        sp_conv = self.conv(sp_output)
        #out = self.out(sp_output_t)


        #print('C O sp_output_t size :', sp_output_t.size(), 'sigmoid size=', out.size())
        #sp_output_aux = Variable(sp_output_t.data.new(sp_output_t.size(0), sp_output_t.size(1), 1).zero_())

        # print('sp_output_aux :', sp_output_aux.shape)
        # print('sp_output_t :', sp_output_t.shape)

        #predict_support = torch.cat([sp_output_aux, sp_output_t], dim=-1).contiguous()


        # print('predict_support :', predict_support.shape)
