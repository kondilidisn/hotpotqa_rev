...
class SPModel(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        # self.sentence_cnn = nn.Conv1d(400, config.char_hidden *2, 5)
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        # ques_ch:  torch.Size([29, 8, 16])

        # print('CNN output! shape :', self.char_cnn(ques_ch.permute(0, 2, 1).contiguous()).size())
        
        # CNN output! shape : torch.Size([29, 100, 12])
        ...
        # print('Unsqueezed context word :!  ', context_word.unsqueeze(-1))
        ...
        # print('Sentence CNN input shape (context):',context_output.contiguous().size())
        # print('Sentence CNN input shape (question) :',ques_output.contiguous().size())

        # context_output = context_output.view(bsz * para_size, 1, 400)
        # ques_output = ques_output.view(bsz * ques_size, 1, 400)
        # print('Sentence CNN input shape (context):',context_output.contiguous().size())
        # print('Sentence CNN input shape (question) :',ques_output.contiguous().size())

        # context_output = self.sentence_cnn(context_output.contiguous()).max(dim=-1)[0].view(bsz, para_size, -1)
        # ques_output = self.sentence_cnn(ques_output.contiguous()).max(dim=-1)[0].view(bsz, ques_size, -1)

        # print('Sentence CNN OUTPUT shape (context):',context_output.size())
        # print('Sentence CNN OUTPUT shape (question) :',ques_output.size())
        ...
        # print('Initial output shape :', output.size())
        ...
        sp_probabilities = torch.sigmoid(sp_output_t)

        # apply threshold
        sp_probabilities = (sp_probabilities > self.config.sp_threshold).float() * sp_probabilities


        # print('predict_support shape :', predict_support.size())

        # print('sp_probabilities shape :', sp_probabilities.size())

        # print()

        # print("sp_output Before all_mapping mat mul :", sp_output.size())


        # print("sp_output_t shape :", sp_output_t.size())
        # print("sp_output_aux shape :", sp_output_aux.size())

        # print("all_mapping shape :", all_mapping.size())


        sp_output = torch.matmul(all_mapping, sp_output)

        maped_sp_probabilities = torch.matmul(all_mapping, sp_probabilities)

        # print('!!! maped_sp_probs shape :', maped_sp_probabilities.size())

        # print("Sp_out after all_mapping mat mul", sp_output.size())

        output_start = torch.cat([output, sp_output], dim=-1)

        # print("output_start shape PRIN:", output_start.size())
        # print()

        repeated_maped_sp_probabilities = maped_sp_probabilities.repeat(1, 1, self.config.hidden*3)
        
        # print('repeated_maped_sp_probabilities shape :', repeated_maped_sp_probabilities.size())

        output_start = repeated_maped_sp_probabilities * output_start
        ...
        # multiply support fact probabilities with start and end token probabilities
        # (in order to amplify probabilities of support fact sentences)
        logit1 = maped_sp_probabilities.squeeze(2)  * logit1
        logit2 = maped_sp_probabilities.squeeze(2)  * logit2
