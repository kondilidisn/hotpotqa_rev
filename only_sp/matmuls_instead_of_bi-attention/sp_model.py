...
class SPModel(nn.Module):
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False):
        ...
        # print('unsqueezed sizes :')
        # print(context_output[0,:,:].unsqueeze(2).size())
        # print(ques_output[0,:,:].unsqueeze(0).size())

        # context_output size : torch.Size([2, 1307, 160])
        # ques_output size : torch.Size([2, 30, 160])
        # unsqueezed sizes :
        # torch.Size([2, 1307, 1, 160])
        # torch.Size([2, 1, 30, 160])

        joint_output = torch.matmul(context_output, ques_output.permute(0, 2, 1))
        joint_output = torch.matmul(joint_output, ques_output)


        # temp = torch.bmm(context_output[0,:,:].unsqueeze(2), ques_output[0,:,:].unsqueeze(0))

        # temp = torch.ger(context_output[0,:,:], ques_output[0,:,:])
        ...
        sp_output = torch.matmul(all_mapping.permute(0, 2, 1), joint_output)
        ...
        # # print('start_mapping oles oi lekseis, prwth periodos')
        # # print('start_mapping :', start_mapping[0,:,:].shape)
        # # print(torch.sum(start_mapping[0,:,:], dim = 1))

        # # print('Inside forward:')
        # # print('sp_output :', sp_output.shape)
