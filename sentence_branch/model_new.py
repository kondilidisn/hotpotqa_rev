...
class SPModel(nn.Module):
    def __init__(self, config, word_mat, char_mat):
        ...
        self.linear_sp = nn.Linear(config.hidden*2+80, 1)
    ...
    def mean_pooling_sentence_embedings(self,all_mapping,word_embeting_array,save):
...
#        logging("attension linear output"+str(output.shape),self.config.save)
        ...
        sentence_embeddings = self.mean_pooling_sentence_embedings(all_mapping ,output ,self.config.save)
