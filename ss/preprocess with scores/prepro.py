...
def _process_article(article, config):
    ...
    def _process(sent, is_sup_fact, par_score, is_title=False):
        nonlocal text_context, context_tokens, context_chars, offsets, start_end_facts, flat_offsets, sentence_scores
        ...
        sentence_scores.append(par_score)
    ...
    for para in paragraphs:
        ...
        cur_title, par_score, cur_para = para[0], para[1], para[2]
        ...
        _process(prepro_sent(cur_title), False, par_score, is_title=True)
        ...
        for sent_id, sent in enumerate(cur_para):
            ...
            if is_sup_fact:
                sp_fact_cnt += 1
            _process(prepro_sent(sent), is_sup_fact, par_score)
    ...
    example = {'context_tokens': context_tokens,'context_chars': context_chars, 'ques_tokens': ques_tokens, 'ques_chars': ques_chars, 'y1s': [best_indices[0]], 'y2s': [best_indices[1]], 'id': article['_id'], 'start_end_facts': start_end_facts, 'sentence_scores' : sentence_scores}
    eval_example = {'context': text_context, 'spans': flat_offsets, 'answer': [answer], 'id': article['_id'], 'sentence_scores' : sentence_scores, 
...
def process_file(filename, config, word_counter=None, char_counter=None):
    ...
    # i = 0
    # for article in data:
    # 	outputs.append(_process_article(article, config))

    # 	# if i == 0:
    # 		# print('Example example :', outputs )
...
def build_features(config, examples, data_type, out_file, word2idx_dict, char2idx_dict):
    ...
    for example in tqdm(examples):
        ...
        datapoints.append({'context_idxs': context_idxs,
            ...
            'y2': y2,
            'id': example['id'],
            'sentence_scores':  torch.FloatTensor(example['sentence_scores']),
...
def prepro(config):
    ...
    print('H build features ta swzei sto :',record_file )
    print('H save ta swzei sto :', eval_file)
